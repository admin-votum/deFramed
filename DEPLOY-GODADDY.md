...
The header and four editable lines are stored in `content.json` under `settings`. Windows sort by `publishedAt` with newest first. The browser editor exposes the same site-text fields locally; public changes still require uploading the updated `content.json`.
...
2. Add a window to `content.json` with a title and `"video": "media/room-light.mp4"`.
